...
async function resolveLeague(supabase, { source, externalId, name, country, logo }) {
  ...
  if (!config) {
    console.warn(
      `[resolver] liga "${name}" (${source}:${externalId}) não está em config/leagues.js — pulando.`
    );
    return null;
  }
  ...
  const { data: league, error } = await supabase
    .from('leagues')
    .upsert(
      { canonical_key: config.key, name: config.name, country: config.country, logo },
...
async function resolveTeam(supabase, { source, externalId, name, logo, leagueId }) {
  ...
  if (!match) {
    ...
    if (match) {
      console.log(`[resolver] match aproximado: "${name}" (${source}) ~= "${match.name}" (existente)`);
    }
  }
